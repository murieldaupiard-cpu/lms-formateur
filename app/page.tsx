  ...
  {view==="home"?<>
   ...
   <section className="studio-list">
    ...
    <div className="creator-workspace">
      ...
      <div className={"creation-row "+(step===3?"active-step":"")} onClick={()=>setStep(3)}><span className="row-num">03</span><div className="palette-step"><small>ÉTAPE 3</small><h3>Séquences & activités</h3><p>Organisez les séquences pédagogiques, activités d’apprentissage, ressources et évaluations.</p>{step===3&&<div className="sequence-builder" onClick={e=>e.stopPropagation()}><label>Intitulé de la séquence<input value={sequenceName} onChange={e=>setSequenceName(e.target.value)} placeholder="Ex. Séquence 1"/></label><div className="builder-block"><div><small>SÉQUENCE 01</small><h4>{sequenceName||"Nouvelle séquence"}</h4></div><button type="button">＋ AJOUTER UN CHAPITRE</button></div><div className="activity-picker"><span>AJOUTER UNE ACTIVITÉ D’APPRENTISSAGE</span><div>{["Cours","Quiz","Casino Game","Time’s Up","Questionnaire"].map(a=><button type="button" key={a} onClick={()=>setActivityName(a)}>＋ {a}</button>)}</div>{activityName==="Cours"?<div className="course-composer"><div className="course-layouts"><span>PRÉSENTATION DU COURS</span><div><button type="button" className={courseLayout==="hero"?"chosen":""} onClick={()=>setCourseLayout("hero")}>01 · DOSSIER / CHIFFRES CLÉS</button><button type="button" className={courseLayout==="program"?"chosen":""} onClick={()=>setCourseLayout("program")}>02 · INTRODUCTION / PROGRAMME</button><button type="button" className={courseLayout==="cards"?"chosen":""} onClick={()=>setCourseLayout("cards")}>03 · CARTES / CATÉGORIES</button></div></div><div className={`course-preview ${courseLayout}`}><small>APERÇU DU COURS</small><h4>{courseLayout==="hero"?"Titre principal":courseLayout==="program"?"Bienvenue dans votre cours":"Le contenu par catégorie"}</h4><p>Ajoutez votre contenu pédagogique puis enrichissez la page avec les ressources de votre choix.</p><div className="mock-content">{courseLayout==="hero"?<><i/><i/><i/><i/></>:courseLayout==="program"?<><b>01</b><span>Première partie du cours</span><b>02</b><span>Deuxième partie du cours</span></>:<><i/><i/><i/><i/><i/><i/></>}</div></div><div className="media-picker"><span>INTÉGRER UNE RESSOURCE</span><div><button type="button">▧ VISUEL</button><button type="button">▣ PHOTO</button><button type="button">▶ VIDÉO</button><button type="button">PDF DOCUMENT</button><button type="button">↗ LIEN</button><button type="button">♫ AUDIO</button></div><small>Image, vidéo, PDF, document, lien ou audio pourront être intégrés directement dans le cours.</small></div></div>:activityName&&<p className="activity-added">✓ {activityName} ajouté à la séquence</p>}</div><div className="builder-actions"><button type="button" className="secondary-step">＋ AJOUTER UNE SÉQUENCE</button><button type="button" className="next-step" onClick={()=>setStep(4)}>CONTINUER →</button></div></div>}</div><b>→</b></div>
